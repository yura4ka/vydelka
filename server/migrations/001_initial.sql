...
CREATE TABLE translations (
  item_id UUID NOT NULL REFERENCES translation_items(id),
...
CREATE TABLE filter_variants (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  slug VARCHAR(64) NOT NULL,
  variant_translation_item UUID NOT NULL REFERENCES translation_items(id),
  filter_id UUID NOT NULL REFERENCES filters(id),
  UNIQUE(slug, filter_id)
);

CREATE TABLE product_filters (
  product_id UUID NOT NULL REFERENCES products(id),
  variant_id UUID NOT NULL REFERENCES filter_variants(id),
